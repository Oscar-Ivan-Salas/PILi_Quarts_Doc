...
export function AnimatedAIChat() {
    ...
    const commandSuggestions: CommandSuggestion[] = [
        {
            ...
            description: "Crear una nueva cotización rápida",
            prefix: "/cotizacion-simple"
        },
        {
            icon: <FileText className="w-4 h-4 text-yellow-500" />,
            label: "Cotización Compleja",
            description: "Iniciar flujo de proyecto detallado",
            prefix: "/cotizacion-compleja"
        },
        {
            icon: <FolderOpen className="w-4 h-4 text-blue-400" />,
            label: "Proyecto Simple",
            description: "Gestión básica de proyecto",
            prefix: "/proyecto-simple"
        },
        {
            icon: <FolderOpen className="w-4 h-4 text-indigo-500" />,
            label: "Proyecto Complejo",
            description: "Gestión avanzada de proyecto",
            prefix: "/proyecto-complejo"
        },
        {
            icon: <BarChart3 className="w-4 h-4 text-green-400" />,
            label: "Informe Simple",
            description: "Reporte básico de estado",
            prefix: "/informe-simple"
        },
        {
            icon: <BarChart3 className="w-4 h-4 text-emerald-600" />,
            label: "Informe Complejo",
            description: "Análisis financiero detallado",
            prefix: "/informe-complejo"
        },
    ];
    ...
    const selectCommandSuggestion = (index: number) => {
        ...
        if (selectedCommand.prefix === "/cotizacion-simple") {
            setActiveSection('cotizacion-simple');
            setShowCommandPalette(false);
            return;
        }
        if (selectedCommand.prefix === "/cotizacion-compleja") {
            setActiveSection('cotizacion-compleja');
            setShowCommandPalette(false);
            return;
        }
        if (selectedCommand.prefix === "/proyecto-simple") {
            setActiveSection('proyecto-simple');
            setShowCommandPalette(false);
            return;
        }
        if (selectedCommand.prefix === "/proyecto-complejo") {
            setActiveSection('proyecto-complejo');
            setShowCommandPalette(false);
            return;
        }
        if (selectedCommand.prefix === "/informe-simple") {
            setActiveSection('informe-simple');
            setShowCommandPalette(false);
            return;
        }
        if (selectedCommand.prefix === "/informe-complejo") {
            setActiveSection('informe-complejo');
            setShowCommandPalette(false);
            return;
        }

        // Fallback or text insertion
        setValue(selectedCommand.prefix + ' ');
        setShowCommandPalette(false);
