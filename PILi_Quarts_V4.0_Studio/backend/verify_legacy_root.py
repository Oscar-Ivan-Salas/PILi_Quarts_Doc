...
# Add CWD to path if not present
if os.getcwd() not in sys.path:
    sys.path.insert(0, os.getcwd())

# Configure Logging
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("LegacyVerifierRoot")
...
def test_legacy_pipeline():
    # Output dir in backend/legacy_test_output
        ...
        "capitulos": [
            {
                "nombre": "Hardware",
                "items": [
                    {"descripcion": "Servidor Dell", "cantidad": 2, "unidad": "UND", "precio_unitario": 12500.00},
                    {"descripcion": "Switch Cisco", "cantidad": 1, "unidad": "UND", "precio_unitario": 2500.00}
                ]
            }
        ]
    }
    ...
    logger.info("🚀 Starting Legacy Pipeline Test (from Root)...")
