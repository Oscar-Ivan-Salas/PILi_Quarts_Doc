...
class CotizacionComplejaGenerator(BaseDocumentGenerator):
    ...
    def generar(self, ruta_salida):
        ...
        # 1. HEADER (Logo y Empresa) - SIEMPRE PRIMERO
        try:
            logger.info("👉 Ejecutando: Header Básico")
            self._agregar_header_basico()
        except Exception as e:
            logger.error(f"❌ ERROR EN HEADER: {e}", exc_info=True)

        # 2. TÍTULO E INFORMACIÓN
        try:
            logger.info("👉 Ejecutando: Título e Info")
            self._agregar_titulo()
            self._agregar_info_general()
            self._agregar_alcance()
        except Exception as e:
            logger.error(f"❌ ERROR EN INFO GENERAL: {e}", exc_info=True)
        ...
        # 5. RESTO DEL DOCUMENTO
        methods = [
            # Header, Titulo, Info, Alcance, Capitulos, Totales YA EJECUTADOS
