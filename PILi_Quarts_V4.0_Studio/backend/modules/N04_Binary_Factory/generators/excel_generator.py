...
class ExcelGenerator:
    ...
    def _set_default_colors(self):
        """Mantiene los colores Tesla por defecto"""
    ...
    def apply_branding(self, branding_data: Dict[str, Any]):
        """Actualiza la paleta de colores basada en ADN Visual"""
        if not branding_data:
            return
            
        color_hex = branding_data.get('color_hex') or branding_data.get('primaryColor')
        if color_hex:
            # Limpiar # si existe
            clean_hex = color_hex.lstrip('#').upper()
            if len(clean_hex) == 6:
                logger.info(f"🎨 Excel: Aplicando ADN Visual Dinámico: {clean_hex}")
                self.COLOR_PRIMARY = clean_hex
                # Derivar secundario (aproximación simple oscureciendo)
                # O podríamos usar el secondaryColor si viene en settings
                sec_hex = branding_data.get('secondaryColor')
                if sec_hex:
                    self.COLOR_SECONDARY = sec_hex.lstrip('#').upper()
        ...
        if logo_path:
            ...
            logger.info(f"🎨 Excel: Logo ADN Visual registrado: {logo_path}")

    def _get_currency_format(self, currency_code: str) -> str:
        """Retorna el formato de celda de Excel para la moneda específica"""
        formats = {
            'PEN': '"S/" #,##0.00',
            'USD': '"$" #,##0.00',
            'EUR': '"€" #,##0.00'
        }
        return formats.get(currency_code, '"S/" #,##0.00')

    def _update_styles(self):
        """Regenera los objetos de estilo de openpyxl con los colores actuales"""
    ...
    def generate_cotizacion(
        ...
        # Logo Block (TESLA) con SMART SCALE
        if self.logo_path and Path(self.logo_path).exists():
            try:
                ...
                with PILImage.open(self.logo_path) as pil_img:
                    w, h = pil_img.size
                    aspect = w / h
                    # Ajustar a un alto de ~60px
                    target_h = 60
                    target_w = target_h * aspect
                    
                    if target_w > 180: # Si es muy ancho, limitar a 180px
                        target_w = 180
                ...
                ws.add_image(img, 'B1') # Centrado en B1 mejor
                ws.merge_cells('A1:C2')
                for r in ws['A1:C2']:
                    for cell in r:
                        cell.fill = self.header_fill
            except Exception as e:
                logger.error(f"Error Smart Scale Excel: {e}")
                ws.merge_cells('A1:C2')
                ws['A1'] = "TESLA"
        ...
        for idx, item in enumerate(items, 1):
            ...
            cur_format = self._get_currency_format(data.get('settings', {}).get('currency', 'PEN'))
            ws.cell(row=row, column=5).number_format = cur_format
            ws.cell(row=row, column=6).number_format = cur_format
        ...
        for label, val in labels:
            ...
            ws[f'F{row}'] = val
            cur_format = self._get_currency_format(data.get('settings', {}).get('currency', 'PEN'))
            ws[f'F{row}'].number_format = cur_format
