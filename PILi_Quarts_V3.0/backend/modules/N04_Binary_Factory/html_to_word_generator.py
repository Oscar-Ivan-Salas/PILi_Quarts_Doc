...
class HTMLToWordGenerator:
    ...
    def render_html(self, tipo_plantilla: str, datos: Dict[str, Any]) -> str:
        """
        Renderiza la plantilla HTML con los datos (Jinja2/String Replace).
        Retorna el HTML procesado como string.
        """
        html = self._cargar_plantilla(tipo_plantilla)
        
        # Preparar datos completos (Simulando lo que hacen los metodos especificos)
        # Esto es genérico, pero los generadores específicos tienen lógicas de mapeo.
        # Mejor estrategia: Usar los metodos especificos pero detenerse antes de convertir?
        # O centralizar el mapeo de datos aquí?
        
        # Por simplicidad y robustez, usaremos _reemplazar_variables directamente.
        # Asumimos que 'datos' ya viene con las claves correctas (pre-mapeado en index.py o caller).
        return self._reemplazar_variables(html, datos)
    ...
    def _convertir_html_a_word(self, html: str, ruta_salida: Path) -> Path:
        """
        Convertir HTML a documento Word preservando el "Frame" de Tesla.
        """
        # Ruta al template maestro que tiene los encabezados y pies de página
        template_maestro = self.plantillas_dir.parent / "master_tesla.docx"
        
        if template_maestro.exists():
            doc = Document(str(template_maestro))
            logger.info(f"🎨 Usando Master Template para Word: {template_maestro.name}")
        else:
            doc = Document()
            logger.warning(f"⚠️ Master Template no encontrado en {template_maestro}. Usando documento vacío.")
        ...
        logger.info(f"✅ Documento Word generado con Estilo Tesla: {ruta_salida}")
        return ruta_salida


    def generar_word_desde_html_directo(self, html: str, ruta_salida: Path) -> Path:
        """
        Método de Fidelidad Espejo: Convierte HTML editado directamente a Word.
        """
        logger.info("🔄 Generando Word desde HTML directo (Mirror HQ)...")
        if not ruta_salida.parent.exists():
            ruta_salida.parent.mkdir(parents=True, exist_ok=True)
            
        return self._convertir_html_a_word(html, ruta_salida)
